...
**SAM-112** is one of the **Special Air Mission (SAM)** callsigns reported in the investigation notes for the US government VIP flight tied to tail number **99-0404** — the aircraft associated with **Defense Secretary Pete Hegseth** around the September 8–9, 2025 Fort Huachuca window. The source lists this flight as *"SAM-702 (or SAM-000 or SAM-112)"*, so SAM-112 is a reported-but-uncertain callsign reading for the same airframe covered on the [SAM 99-0404 Flight](/Planes/SAM-99-0404) page. This page tracks the SAM-112 callsign thread on its own so the identifier is searchable and its open questions are recorded. Nothing here alleges wrongdoing by any person.
