...
| FlightAware AeroAPI v4 | `aeroapi.flightaware.com/aeroapi/history/flights/REG?ident_type=registration` — **max 7-day span per call**; run `/aircraft/REG/blocked` first. CORRECTED 2026-09-14: this row used to name `/aircraft/REG/flights`, which is not an AeroAPI v4 resource | **to 2011** | `AEROAPI_KEY` | **held since 2026-09-14.** Raw responses go to the private `all` repo (`proprietary/code/private_store.js`) and are published only as restatements in `proprietary/data/flightaware_restated/`; the client's spend guard refuses past $90/month |
