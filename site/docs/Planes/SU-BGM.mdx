...
* **Color nickname:** None. Unlike SU-BTT (blue) and SU-BND (yellow), Candace Owens's color-coded shorthand assigned **no color label** to SU-BGM; it "appears in flight logs" without one.
...
## Broader claimed tracking window (2022–2025)

While the two tail-specific legs above are both spring 2025, the user flight logs frame SU-BGM within a wider claimed window. Per the raw-log notes, the Israel/Egyptian fleet's alleged Kirk-event overlaps are said to span **2022 through 2025**, with "multiple instances" of international routing into US locations — including potential Provo-area activity — across those years. SU-BGM is grouped into that multi-year pattern by association rather than by a documented standalone leg outside the April 2025 entries.

This broader 2022–2025 framing is, per X OSINT, the same disputed overlap-counting that critics (Liz Wheeler, the FlightRadar skeptic-audit) have challenged for the lead jets — see the [Israel/Egyptian Plane Armada](/Planes/Egyptian-Armada) hub for the full, contested picture. No 2022–2024 SU-BGM leg has been published with a date, route, or source; the multi-year range is an attributed claim, not a demonstrated tail-specific record.
