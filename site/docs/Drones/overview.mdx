...
description: "This page summarizes what has been reported and claimed about drones and unmanned systems in connection with the Charlie Kirk case. It covers two main areas…"
keywords:
  - Charlie Kirk
...
## Documented and reported findings (mixed sources)

Research compiled from mainstream reporting and open-source posts includes:

- **UVU did not deploy drones for event security** on September 10, 2025, per AP and other outlets — six campus officers were monitoring the event instead. [Source: AP via Police1/ABC News coverage cited in project research.]
- **Post-incident drone footage** from news organizations (e.g., Reuters) documented the rooftop shooter position and deserted campus after the shooting — distinct from any pre-event security drone program.
- **UVU has a drone program** but no drones were used to prevent the shooting, per commentary citing campus policy.
- **Citizen-investigator claims** include: a military spy plane capable of launching drones observed near UVU ([N1098L](/Planes/N1098L/overview)); drones spotted at the event with FOIA requests filed for logs; an unknown flying object on video ~30 seconds after the shot; and reports of fast-moving objects over the site. **These are unverified OSINT claims** requiring official records to evaluate.
...
* Who received the drone video feeds over UVU and N1098L spy‑plane telemetry and crew manifests and the FAA drone authorizations issued for UVU airspace that day are things that the [Charlie Kirk Investigation Laws](/Fix/overview) may result in powerful truths coming out that aren't out yet.
