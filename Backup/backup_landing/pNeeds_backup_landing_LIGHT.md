...
# Product Requirements — Backup Posts Landing Page (Light Mode)
# Page: backup_landing | URL: /

**Product Manager Voice — UX Designer Note:** This document defines functional and content requirements. Visual implementation must use the light mode palette. The UX designer is responsible for translating these requirements into polished visual designs.
...
- Scroll behavior: gains solid white background + border-bottom on scroll
...
- Section background: #F7F9F9 (slight contrast from hero)
...
- Creator card: highlighted with 2px blue border #1D9BF0 + "Most Popular" pill badge
...
- Background: #EBF5FF (light blue tint, distinct from rest of page)
...
### Button States
- Primary button: default, hover (#1A8CD8), pressed (#1570B8), disabled (#EFF3F4 bg)
- Ghost button: default, hover (#F7F9F9 bg)

### Pricing Card States
- Default: #F7F9F9 bg, 1px #EFF3F4 border
- Highlighted (Creator): white bg, 2px #1D9BF0 border
- Hover: box-shadow 0 4px 12px rgba(15,20,25,0.10), translateY(-2px), transition 200ms ease

### FAQ Accordion
- Collapsed: question + chevron, min 48px row height
- Expanded: answer visible, chevron rotated 90°, 200ms transition

### Navigation States
- Unauthenticated: Login + Get Started Free
- Authenticated: user avatar dropdown + Go to Dashboard
- Scrolled page: nav bar gains 1px solid #EFF3F4 border-bottom
