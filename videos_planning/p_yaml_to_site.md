...
A banned video gets no page and no poster, is never placed on a topic page, and
any page or poster that already exists for it is DELETED. It is never removed
from {HIERARCHY_FILE} — that file is the complete private record and only grows.
...
regeneration. When a run finds material that should not be published, add its
identity to {EXCLUDE_FILE} with a reason, and record it in {FINDINGS_FILE} so
the hierarchy can drop the entry at its source.

If {EXCLUDE_FILE} does not exist yet, create it with a header comment
explaining the format before the first page is written.
...
Idempotent and safe:
  ...
  * A video excluded in {EXCLUDE_FILE} has no video page to link to — skip it
...
* Read {EXCLUDE_FILE} (create it with a header comment if missing).
...
* For every video entry:
    * Skip it entirely if its identity is in {EXCLUDE_FILE}, and delete any
      page or poster that already exists for it.
...
* Walk every video in the YAML index. Skip any whose identity is in
  {EXCLUDE_FILE}. For the rest, derive the video-page URL from video_page
